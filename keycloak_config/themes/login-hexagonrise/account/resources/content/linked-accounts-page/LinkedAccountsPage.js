...
class LinkedAccountsPage extends React.Component {
  ...
  linkAccount(account) {
    const url = '/linked-accounts/' + account.providerName;
    /**Modificado por nosotros (CUSTOM) */
    const urlParams = new URLSearchParams(window.location.search); 
    const redirectUri = urlParams.get("redirect_uri") || createRedirect(this.props.location.pathname);
    this.context.doGet(url, {
      ...
      /**Modificado por nosotros (CUSTOM) */
      window.top.location.href = response.data.accountLinkUri;
    });
  }
  render() {
