...
class SigningInPage extends React.Component {
  ...
  renderCredTypeTitle(credContainer, keycloak, category) {
    ...
    return /*#__PURE__*/React.createElement(React.Fragment, {
      ...
      className: "pf-c-button pf-m-link",
      type: "button",
      /* actualizado por nosotros (CUSTOM)
      onClick: () => setupAction.execute()
      */
      onClick: () => {
            const urlParams = new URLSearchParams(window.location.search); 
            const redirectUri = urlParams.get("redirect_uri");
            const url = setupAction.keycloak.keycloakAuth.createLoginUrl({action: setupAction.action, redirectUri});
    ...
    if (this.props.updateAction) {
      ...
      /* actualizado por nosotros (CUSTOM)
        ...
        onClick: () => {
            const urlParams = new URLSearchParams(window.location.search); 
            const redirectUri = urlParams.get("redirect_uri");
            const url = this.props.updateAction.keycloak.keycloakAuth.createLoginUrl({action: this.props.updateAction.action, redirectUri});
            window.top.location.href = url
        } 
